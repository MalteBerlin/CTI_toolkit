...
public class Scanner_BTLE {//Constructor
...
//Needed for the scanning functionality to be performed
    public Scanner_BTLE(MainActivity mainActivity, long scanPeriod, int signalStrength) {//Constructor also says to run within the main activity
        //Constructor
        ma = mainActivity;
        ...
        final BluetoothManager bluetoothManager = (BluetoothManager) ma.getSystemService(Context.BLUETOOTH_SERVICE);//Create a reference to the bluetooth service to the phone
...
// This is Deprecated in API 28
    ...
    public void start() {//Used prior to starting the scan to ensure that the bluetooth adapter is enabled
        ...
        if (!Utils.checkbluetooth(mbluetoothAdapter)) {
            Utils.requestUserBluetooth(ma);//Start the bluetooth from the main activity but passed through the utils function
            ma.stopScan();//Stops scanning
        }
        if(isLocationEnabled(getApplicationContext()) == false)
        {
            Utils.requestUserLocation(ma);
            ma.stopScan();
        }
    ...
    private void scanLeDevice(final boolean enable) {//The function that finds the BTLE devices it requires scanning to be off and for an enable true to be passed in
        if ((enable) && (!mScanning)) {
            Utils.toast(ma.getApplication(), "Started ble scan");//Inform the user that the device has started a ble scan
            mHandler.postDelayed(new Runnable() {
                @Override
                public void run() {
                    Utils.toast(ma.getApplication(), "Stopping ble scan");//Inform the user when the scan has been stopped

                    mScanning = false;//Set scanning to be false
                    mbluetoothAdapter.stopLeScan(mleScanCallback);//Stop the call back
                    ma.stopScan();//Stop scanning
                    //Log.d(TAG2, "Stopping ble scan");

                    if (scan_type == true)//Set up the scanning duration based upon the slider for perma scan
                    {
                        //scan_duration = 300000;//5 mins
                        scan_duration = 86400000;//24 Hours
                    } else if ((scan_type == false))//Use a default scanning period
                    {
                        scan_duration = 86400000;//24 Hours
                        //scan_duration = scanPeriod;
                    }
                }
            }, scan_duration);
            ...
            //Log.d(TAG2, "scanLeDevice: Starting callback");
        } else {
            mScanning = false;
            mbluetoothAdapter.stopLeScan(mleScanCallback);
            //Log.d(TAG2, "scanLeDevice: Stopping scan");
        }
    }
        ...
        public void onLeScan(final BluetoothDevice device, int rssi, final byte[] scanRecord) {// The device has been found and passed in the device, power and advert respectively.
            ...
            mHandler.post(new Runnable() {//Once received the lescan can be run again
                ...
                public void run() {//Add the device to the list of found devices depending upon the type of device, either a board of the phone
                    ...
                    if (device.getAddress().equals(global_settings.Get_Bound_device())) {//Filter by centrica address
                        ma.addDevice_Sensor_board(device, new_rssi, new_scanRecord);
                    }
                }
            });
        }
